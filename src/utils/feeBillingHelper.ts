...
export function getEvaluatedFeeStatus(
  ...
  if (rawStatus !== undefined && rawStatus !== null) {
    ...
    if (sUpper === "UNPAID") return "UNPAID";
    if (sUpper === "NA" || sUpper === "N/A") return "N/A";
  }

  // Rule 1: Current or Future Month -> N/A
  if (!isPastMonth(monthYearStr, currentDate)) {
    return "N/A";
  }

  // Rule 2: Convert N/A to UNPAID only if:
  // - Month has ended (handled by isPastMonth above)
  // - AND Today >= 3rd day of current month
  // - AND Student has attendance in that month
  const isAfterSecondDay = currentDate.getDate() >= 3;
  const hasAttendance = hasAttendanceInMonth(student, monthYearStr);

  if (isAfterSecondDay && hasAttendance) {
    return "UNPAID";
  }

  // Rule 3: Otherwise keep N/A
  return "N/A";
}
